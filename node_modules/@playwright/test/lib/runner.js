...
class Runner {
  constructor(configCLIOverrides) {
    ...
    this._plugins = [];
    this._watchRepeatEachIndex = 0;
    this._watchJobsQueue = Promise.resolve();
  ...
  async _collectFiles(testFileFilters, projectNames) {
    const testFileFilter = testFileFilters.length ? (0, _util2.createFileMatcher)(testFileFilters.map(e => e.re || e.exact || '')) : () => true;
    let projectsToFind;
    let unknownProjects;
  ...
  async _run(options) {
    ...
    if (duplicateTitlesError) fatalErrors.push(duplicateTitlesError); // 3. Filter tests to respect line/column filter.

    filterByFocusedLine(preprocessRoot, testFileFilters); // 4. Complain about only.
    ...
    if (!options.listOnly) filterOnly(preprocessRoot); // 6. Generate projects.
    ...
    if (!total && !options.passWithNoTests) fatalErrors.push(createNoTestsError()); // 8. Compute shards.
    ...
    config._testGroupsCount = testGroups.length; // 9. Report begin

    (_this$_reporter$onBeg = (_this$_reporter4 = this._reporter).onBegin) === null || _this$_reporter$onBeg === void 0 ? void 0 : _this$_reporter$onBeg.call(_this$_reporter4, config, rootSuite); // 10. Bail out on errors prior to running global setup.
  ...
  async _watch(options) {
    var _this$_reporter$onBeg2, _this$_reporter8;

    const config = this._loader.fullConfig(); // 1. Create empty suite.


    const rootSuite = new _test.Suite('', 'root'); // 2. Report begin.

    (_this$_reporter$onBeg2 = (_this$_reporter8 = this._reporter).onBegin) === null || _this$_reporter$onBeg2 === void 0 ? void 0 : _this$_reporter$onBeg2.call(_this$_reporter8, config, rootSuite); // 3. Remove output directores.
    ...
    const runAndWatch = async () => {
      ...
      for (const files of filesByProject.values()) files.forEach(file => allTestFiles.add(file)); // 6. Trigger 'all files changed'.


      await this._runAndReportError(async () => {
        await this._runModifiedTestFilesForWatch(progress, options, allTestFiles);
      }, result); // 7. Start watching the filesystem for modifications.

      await this._watchTestFiles(progress, options);
    };

    try {
      var _progress$dispatcher;

      const sigintWatcher = new _sigIntWatcher.SigIntWatcher();
      await Promise.race([runAndWatch(), sigintWatcher.promise()]);
      if (!sigintWatcher.hadSignal()) sigintWatcher.disarm();
      progress.canceled = true;
      await ((_progress$dispatcher = progress.dispatcher) === null || _progress$dispatcher === void 0 ? void 0 : _progress$dispatcher.stop());
    } finally {
      await (globalTearDown === null || globalTearDown === void 0 ? void 0 : globalTearDown());
    }
  ...
  async _runModifiedTestFilesForWatch(progress, options, testFiles) {
    if (progress.canceled) return;
    const testFileFilters = [...testFiles].map(f => ({
      exact: f,
      line: null,
      column: null
    }));
  ...
  async _watchTestFiles(progress, options) {
    const folders = new Set();

    const config = this._loader.fullConfig();

    for (const project of config.projects) {
      if (options.projectFilter && !options.projectFilter.includes(project.name)) continue;
  ...
  async _removeOutputDirs(options) {
    ...
    try {
      ...
      var _this$_reporter$onErr4, _this$_reporter9;

      (_this$_reporter$onErr4 = (_this$_reporter9 = this._reporter).onError) === null || _this$_reporter$onErr4 === void 0 ? void 0 : _this$_reporter$onErr4.call(_this$_reporter9, (0, _util2.serializeError)(e));
      return false;
    }
  ...
  async _performGlobalSetup(config, rootSuite, projects, result) {
    ...
    this._plugins.push(...(0, _webServerPlugin.webServerPluginsForConfig)(config, this._reporter));
    ...
    await this._runAndReportError(async () => {
      ...
      for (const plugin of this._plugins) {
        ...
        await Promise.race([(_plugin$setup = plugin.setup) === null || _plugin$setup === void 0 ? void 0 : _plugin$setup.call(plugin, config, config._configDir, rootSuite), sigintWatcher.promise()]);
  ...
  async _runAndReportError(callback, result) {
    try {
      ...
      var _this$_reporter$onErr5, _this$_reporter10;

      result.status = 'failed';
      (_this$_reporter$onErr5 = (_this$_reporter10 = this._reporter).onError) === null || _this$_reporter$onErr5 === void 0 ? void 0 : _this$_reporter$onErr5.call(_this$_reporter10, (0, _util2.serializeError)(e));
    }
  }
