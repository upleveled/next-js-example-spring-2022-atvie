...
class WebServerPlugin {
  constructor(options, checkPortOnly, reporter) {
    ...
    this._isAvailable = getIsAvailableFunction(options.url, checkPortOnly, !!options.ignoreHTTPSErrors, (_this$_reporter$onStd = this._reporter.onStdErr) === null || _this$_reporter$onStd === void 0 ? void 0 : _this$_reporter$onStd.bind(this._reporter));
  }

  async setup(config, configDir) {
  ...
  async _startProcess() {
    ...
    launchedProcess.stderr.on('data', line => {
      var _this$_reporter$onStd2, _this$_reporter;

      return (_this$_reporter$onStd2 = (_this$_reporter = this._reporter).onStdErr) === null || _this$_reporter$onStd2 === void 0 ? void 0 : _this$_reporter$onStd2.call(_this$_reporter, '[WebServer] ' + line.toString());
    });
    launchedProcess.stdout.on('data', line => {
      var _this$_reporter$onStd3, _this$_reporter2;

      if (debugWebServer.enabled) (_this$_reporter$onStd3 = (_this$_reporter2 = this._reporter).onStdOut) === null || _this$_reporter$onStd3 === void 0 ? void 0 : _this$_reporter$onStd3.call(_this$_reporter2, '[WebServer] ' + line.toString());
    });
  }
...
async function httpStatusCode(url, ignoreHTTPSErrors, onStdErr) {
  ...
  const requestOptions = isHttps ? { ...commonRequestOptions,
...
const webServer = options => {
  ...
  return new WebServerPlugin(options, false, {
    onStdOut: d => console.log(d.toString()),
    onStdErr: d => console.error(d.toString())
  });
};

exports.webServer = webServer;

const webServerPluginsForConfig = (config, reporter) => {
  ...
  for (const webServerConfig of config._webServers) {
    ...
    const url = webServerConfig.url || `http://localhost:${webServerConfig.port}`; // We only set base url when only the port is given. That's a legacy mode we have regrets about.

    if (shouldSetBaseUrl && !webServerConfig.url) process.env.PLAYWRIGHT_TEST_BASE_URL = url;
    webServerPlugins.push(new WebServerPlugin({ ...webServerConfig,
      url
    }, webServerConfig.port !== undefined, reporter));
  }
