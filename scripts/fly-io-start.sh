...
[ ! -f $VOLUME_PATH/run/postgresql/data/postgresql.conf ] && IS_DATABASE_INITIALIZED=true
...
[ $IS_DATABASE_INITIALIZED == true ] && su postgres -c "initdb -D $VOLUME_PATH/run/postgresql/data/"
...
[ $IS_DATABASE_INITIALIZED == true ] && psql -U postgres postgres --command="CREATE USER $PGUSERNAME PASSWORD '$PGPASSWORD'"
[ $IS_DATABASE_INITIALIZED == true ] && createdb -U postgres --owner=$PGUSERNAME $PGDATABASE
